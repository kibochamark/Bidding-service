import { AuctionService } from '../../Domains/Bidding/auction.service';
import { AuctionParamDto, CreateAuctionDto, UpdateAuctionDto } from './dto';
export declare class AuctionController {
    private auctionService;
    private readonly logger;
    constructor(auctionService: AuctionService);
    getActiveAuctions(): Promise<({
        product: {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            rating: import("@prisma/client-runtime-utils").Decimal;
            description: string;
            title: string;
            categoryId: string;
            condition: import("../../../generated/prisma/enums").ProductCondition;
            images: string[];
            startingPrice: import("@prisma/client-runtime-utils").Decimal;
            currentBid: import("@prisma/client-runtime-utils").Decimal;
            bidsCount: number;
            reservePrice: import("@prisma/client-runtime-utils").Decimal | null;
            buyNowPrice: import("@prisma/client-runtime-utils").Decimal | null;
            startDate: Date;
            endDate: Date;
            isActive: boolean;
            sellerId: string;
            sellerName: string;
            sellerRating: import("@prisma/client-runtime-utils").Decimal;
            reviewCount: number;
            specifications: import("@prisma/client/runtime/client").JsonValue;
        };
        _count: {
            bids: number;
        };
    } & {
        id: string;
        status: import("../../../generated/prisma/enums").AuctionStatus;
        createdAt: Date;
        updatedAt: Date;
        description: string;
        title: string;
        startDate: Date;
        endDate: Date;
        productId: string;
        prizeValue: import("@prisma/client-runtime-utils").Decimal;
        entryFee: import("@prisma/client-runtime-utils").Decimal;
        winnerId: string | null;
        winningBidAmount: import("@prisma/client-runtime-utils").Decimal | null;
        totalBidsCount: number;
        totalRevenue: import("@prisma/client-runtime-utils").Decimal;
    })[]>;
    getAuctionById(params: AuctionParamDto): Promise<{
        product: {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            rating: import("@prisma/client-runtime-utils").Decimal;
            description: string;
            title: string;
            categoryId: string;
            condition: import("../../../generated/prisma/enums").ProductCondition;
            images: string[];
            startingPrice: import("@prisma/client-runtime-utils").Decimal;
            currentBid: import("@prisma/client-runtime-utils").Decimal;
            bidsCount: number;
            reservePrice: import("@prisma/client-runtime-utils").Decimal | null;
            buyNowPrice: import("@prisma/client-runtime-utils").Decimal | null;
            startDate: Date;
            endDate: Date;
            isActive: boolean;
            sellerId: string;
            sellerName: string;
            sellerRating: import("@prisma/client-runtime-utils").Decimal;
            reviewCount: number;
            specifications: import("@prisma/client/runtime/client").JsonValue;
        };
        bids: {
            id: string;
            auctionId: string;
            bidderId: string;
            bidderName: string;
            bidAmount: import("@prisma/client-runtime-utils").Decimal;
            isUnique: boolean;
            isWinning: boolean;
            placedAt: Date;
        }[];
    } & {
        id: string;
        status: import("../../../generated/prisma/enums").AuctionStatus;
        createdAt: Date;
        updatedAt: Date;
        description: string;
        title: string;
        startDate: Date;
        endDate: Date;
        productId: string;
        prizeValue: import("@prisma/client-runtime-utils").Decimal;
        entryFee: import("@prisma/client-runtime-utils").Decimal;
        winnerId: string | null;
        winningBidAmount: import("@prisma/client-runtime-utils").Decimal | null;
        totalBidsCount: number;
        totalRevenue: import("@prisma/client-runtime-utils").Decimal;
    }>;
    getAuctionStats(params: AuctionParamDto): Promise<{
        auction: {
            product: {
                id: string;
                createdAt: Date;
                updatedAt: Date;
                rating: import("@prisma/client-runtime-utils").Decimal;
                description: string;
                title: string;
                categoryId: string;
                condition: import("../../../generated/prisma/enums").ProductCondition;
                images: string[];
                startingPrice: import("@prisma/client-runtime-utils").Decimal;
                currentBid: import("@prisma/client-runtime-utils").Decimal;
                bidsCount: number;
                reservePrice: import("@prisma/client-runtime-utils").Decimal | null;
                buyNowPrice: import("@prisma/client-runtime-utils").Decimal | null;
                startDate: Date;
                endDate: Date;
                isActive: boolean;
                sellerId: string;
                sellerName: string;
                sellerRating: import("@prisma/client-runtime-utils").Decimal;
                reviewCount: number;
                specifications: import("@prisma/client/runtime/client").JsonValue;
            };
            bids: {
                id: string;
                auctionId: string;
                bidderId: string;
                bidderName: string;
                bidAmount: import("@prisma/client-runtime-utils").Decimal;
                isUnique: boolean;
                isWinning: boolean;
                placedAt: Date;
            }[];
        } & {
            id: string;
            status: import("../../../generated/prisma/enums").AuctionStatus;
            createdAt: Date;
            updatedAt: Date;
            description: string;
            title: string;
            startDate: Date;
            endDate: Date;
            productId: string;
            prizeValue: import("@prisma/client-runtime-utils").Decimal;
            entryFee: import("@prisma/client-runtime-utils").Decimal;
            winnerId: string | null;
            winningBidAmount: import("@prisma/client-runtime-utils").Decimal | null;
            totalBidsCount: number;
            totalRevenue: import("@prisma/client-runtime-utils").Decimal;
        };
        totalBids: number;
        uniqueBids: number;
        lowestBid: import("@prisma/client-runtime-utils").Decimal | null;
        highestBid: import("@prisma/client-runtime-utils").Decimal | null;
    }>;
    getAuctionLeaderboard(params: AuctionParamDto): Promise<{
        auction: {
            id: string;
            title: string;
            status: import("../../../generated/prisma/enums").AuctionStatus;
            endDate: Date;
            totalBidsCount: number;
        };
        currentWinner: {
            bidderName: string;
            bidAmount: import("@prisma/client-runtime-utils").Decimal;
        } | null;
        bidStatistics: {
            bidAmount: number;
            count: number;
            isUnique: boolean;
            bidders: string[];
        }[];
    }>;
    createAuction(createAuctionDto: CreateAuctionDto): Promise<{
        product: {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            rating: import("@prisma/client-runtime-utils").Decimal;
            description: string;
            title: string;
            categoryId: string;
            condition: import("../../../generated/prisma/enums").ProductCondition;
            images: string[];
            startingPrice: import("@prisma/client-runtime-utils").Decimal;
            currentBid: import("@prisma/client-runtime-utils").Decimal;
            bidsCount: number;
            reservePrice: import("@prisma/client-runtime-utils").Decimal | null;
            buyNowPrice: import("@prisma/client-runtime-utils").Decimal | null;
            startDate: Date;
            endDate: Date;
            isActive: boolean;
            sellerId: string;
            sellerName: string;
            sellerRating: import("@prisma/client-runtime-utils").Decimal;
            reviewCount: number;
            specifications: import("@prisma/client/runtime/client").JsonValue;
        };
    } & {
        id: string;
        status: import("../../../generated/prisma/enums").AuctionStatus;
        createdAt: Date;
        updatedAt: Date;
        description: string;
        title: string;
        startDate: Date;
        endDate: Date;
        productId: string;
        prizeValue: import("@prisma/client-runtime-utils").Decimal;
        entryFee: import("@prisma/client-runtime-utils").Decimal;
        winnerId: string | null;
        winningBidAmount: import("@prisma/client-runtime-utils").Decimal | null;
        totalBidsCount: number;
        totalRevenue: import("@prisma/client-runtime-utils").Decimal;
    }>;
    finalizeAuction(params: AuctionParamDto): Promise<{
        auction: {
            product: {
                id: string;
                createdAt: Date;
                updatedAt: Date;
                rating: import("@prisma/client-runtime-utils").Decimal;
                description: string;
                title: string;
                categoryId: string;
                condition: import("../../../generated/prisma/enums").ProductCondition;
                images: string[];
                startingPrice: import("@prisma/client-runtime-utils").Decimal;
                currentBid: import("@prisma/client-runtime-utils").Decimal;
                bidsCount: number;
                reservePrice: import("@prisma/client-runtime-utils").Decimal | null;
                buyNowPrice: import("@prisma/client-runtime-utils").Decimal | null;
                startDate: Date;
                endDate: Date;
                isActive: boolean;
                sellerId: string;
                sellerName: string;
                sellerRating: import("@prisma/client-runtime-utils").Decimal;
                reviewCount: number;
                specifications: import("@prisma/client/runtime/client").JsonValue;
            };
            bids: {
                id: string;
                auctionId: string;
                bidderId: string;
                bidderName: string;
                bidAmount: import("@prisma/client-runtime-utils").Decimal;
                isUnique: boolean;
                isWinning: boolean;
                placedAt: Date;
            }[];
        } & {
            id: string;
            status: import("../../../generated/prisma/enums").AuctionStatus;
            createdAt: Date;
            updatedAt: Date;
            description: string;
            title: string;
            startDate: Date;
            endDate: Date;
            productId: string;
            prizeValue: import("@prisma/client-runtime-utils").Decimal;
            entryFee: import("@prisma/client-runtime-utils").Decimal;
            winnerId: string | null;
            winningBidAmount: import("@prisma/client-runtime-utils").Decimal | null;
            totalBidsCount: number;
            totalRevenue: import("@prisma/client-runtime-utils").Decimal;
        };
        winner: null;
        message: string;
    } | {
        auction: {
            product: {
                id: string;
                createdAt: Date;
                updatedAt: Date;
                rating: import("@prisma/client-runtime-utils").Decimal;
                description: string;
                title: string;
                categoryId: string;
                condition: import("../../../generated/prisma/enums").ProductCondition;
                images: string[];
                startingPrice: import("@prisma/client-runtime-utils").Decimal;
                currentBid: import("@prisma/client-runtime-utils").Decimal;
                bidsCount: number;
                reservePrice: import("@prisma/client-runtime-utils").Decimal | null;
                buyNowPrice: import("@prisma/client-runtime-utils").Decimal | null;
                startDate: Date;
                endDate: Date;
                isActive: boolean;
                sellerId: string;
                sellerName: string;
                sellerRating: import("@prisma/client-runtime-utils").Decimal;
                reviewCount: number;
                specifications: import("@prisma/client/runtime/client").JsonValue;
            };
        } & {
            id: string;
            status: import("../../../generated/prisma/enums").AuctionStatus;
            createdAt: Date;
            updatedAt: Date;
            description: string;
            title: string;
            startDate: Date;
            endDate: Date;
            productId: string;
            prizeValue: import("@prisma/client-runtime-utils").Decimal;
            entryFee: import("@prisma/client-runtime-utils").Decimal;
            winnerId: string | null;
            winningBidAmount: import("@prisma/client-runtime-utils").Decimal | null;
            totalBidsCount: number;
            totalRevenue: import("@prisma/client-runtime-utils").Decimal;
        };
        winner: {
            bidderId: string;
            bidderName: string;
            winningBidAmount: import("@prisma/client-runtime-utils").Decimal;
        };
        message: string;
    }>;
    updateAuction(params: AuctionParamDto, updateAuctionDto: UpdateAuctionDto): Promise<{
        product: {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            rating: import("@prisma/client-runtime-utils").Decimal;
            description: string;
            title: string;
            categoryId: string;
            condition: import("../../../generated/prisma/enums").ProductCondition;
            images: string[];
            startingPrice: import("@prisma/client-runtime-utils").Decimal;
            currentBid: import("@prisma/client-runtime-utils").Decimal;
            bidsCount: number;
            reservePrice: import("@prisma/client-runtime-utils").Decimal | null;
            buyNowPrice: import("@prisma/client-runtime-utils").Decimal | null;
            startDate: Date;
            endDate: Date;
            isActive: boolean;
            sellerId: string;
            sellerName: string;
            sellerRating: import("@prisma/client-runtime-utils").Decimal;
            reviewCount: number;
            specifications: import("@prisma/client/runtime/client").JsonValue;
        };
    } & {
        id: string;
        status: import("../../../generated/prisma/enums").AuctionStatus;
        createdAt: Date;
        updatedAt: Date;
        description: string;
        title: string;
        startDate: Date;
        endDate: Date;
        productId: string;
        prizeValue: import("@prisma/client-runtime-utils").Decimal;
        entryFee: import("@prisma/client-runtime-utils").Decimal;
        winnerId: string | null;
        winningBidAmount: import("@prisma/client-runtime-utils").Decimal | null;
        totalBidsCount: number;
        totalRevenue: import("@prisma/client-runtime-utils").Decimal;
    }>;
    deleteAuction(params: AuctionParamDto): Promise<{
        id: string;
        status: import("../../../generated/prisma/enums").AuctionStatus;
        createdAt: Date;
        updatedAt: Date;
        description: string;
        title: string;
        startDate: Date;
        endDate: Date;
        productId: string;
        prizeValue: import("@prisma/client-runtime-utils").Decimal;
        entryFee: import("@prisma/client-runtime-utils").Decimal;
        winnerId: string | null;
        winningBidAmount: import("@prisma/client-runtime-utils").Decimal | null;
        totalBidsCount: number;
        totalRevenue: import("@prisma/client-runtime-utils").Decimal;
    }>;
}
